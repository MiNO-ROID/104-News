...
function toggleSavedArticle(article) {
  if (isSaved(article)) {
    savedArticles = savedArticles.filter(savedArticle => articleId(savedArticle) !== articleId(article));
  } else {
...
function formatDate(dateString) {
  ...
  return Number.isNaN(date.getTime()) ? "Date unavailable" : new Intl.DateTimeFormat("en-NZ", { dateStyle: "medium" }).format(date);
}
...
function createArticleCard(article) {
  ...
  return `<article class="news-card"><img class="news-image" src="${escapeHtml(imageUrl)}" alt="" loading="lazy"><div class="news-content"><div class="article-top-row"><p class="article-source">${source}</p><button class="save-button ${saved ? "saved" : ""}" type="button" data-article="${encodedArticle}" aria-label="${saved ? "Remove saved article" : "Save article"}">Bookmark</button></div><h3>${title}</h3><p class="article-description">${description}</p><div class="article-footer"><time>${formatDate(article.published_at)}</time><a href="${escapeHtml(url)}" target="_blank" rel="noopener noreferrer">Read more</a></div></div></article>`;
}

function addSaveButtonEvents(container) {
  container.querySelectorAll(".save-button").forEach(button => {
    button.addEventListener("click", () => toggleSavedArticle(JSON.parse(decodeURIComponent(button.dataset.article))));
  });
}

function displayNews(articles) {
  const searchTerm = searchInput.value.trim().toLowerCase();
  const filteredArticles = articles.filter(article => [article.title, article.description, article.source].filter(Boolean).join(" ").toLowerCase().includes(searchTerm));
...
function displaySavedArticles() {
  if (!savedArticles.length) {
    savedContainer.innerHTML = '<p class="empty-message">No saved articles yet.</p>';
    return;
  }
...
function showView(viewName) {
  ...
  closeSearchPanel();
  if (viewName === "saved") displaySavedArticles();
}

async function loadNews() {
  setLoading(true);
  clearError();
  const params = new URLSearchParams({ access_key: API_KEY, languages: "en", countries: "nz", limit: "20" });
  if (currentCategory !== "general") params.set("categories", currentCategory);
  try {
    const response = await fetch(`${API_URL}?${params}`);
    const data = await response.json();
    if (!response.ok || data.error) throw new Error(data.error?.message || `Request failed with status ${response.status}`);
...
searchToggle.addEventListener("click", () => { showView("home"); openSearchPanel(); });
closeSearch.addEventListener("click", closeSearchPanel);
document.addEventListener("keydown", event => { if (event.key === "Escape" && !searchPanel.hidden) closeSearchPanel(); });
